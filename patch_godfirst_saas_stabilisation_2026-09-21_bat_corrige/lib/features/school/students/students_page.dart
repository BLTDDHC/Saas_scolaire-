              ...
              Center(
                child: OutlinedButton.icon(
                  onPressed: savingPhoto
                      ? null
                      : () async {
                          final files = await pickStudentPhotos(multiple: false);
                          if (files.isEmpty || !context.mounted) return;
                          final file = files.first;
                          if (!file.isValid) {
                            AppToast.error(context, file.rejectionReason ??
                  ...
                  label: Text(savingPhoto ? 'Enregistrement…' : 'Modifier la photo'),
                ),
              ),
