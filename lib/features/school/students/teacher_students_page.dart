...
class _TeacherStudentsPageState extends State<TeacherStudentsPage> {
  ...
  Widget build(BuildContext context) {
    ...
    final affectations = store
        ...
        .toList();
    final classIds = affectations.map((item) => item.classId).toSet();
    final classes = store
        .getClasses()
    ...
    if (_classId != null && !classIds.contains(_classId)) {
      _classId = null;
    }
    ...
    final students = store
        ...
        .toList()
      ..sort((a, b) {
        final byClass = _normalized(classNames[a.classId] ?? a.className ?? '')
            .compareTo(_normalized(classNames[b.classId] ?? b.className ?? ''));
        ...
        children: [
          ...
          if (students.isEmpty)
            const AppEmptyState(
              ...
              message:
                  'Les élèves apparaîtront ici selon vos affectations et le filtre de classe.',
            )
          else
            LayoutBuilder(
                ...
                return Wrap(
                  ...
                  children: students
                      .map((student) => SizedBox(
                            ...
                            child: _StudentCard(
                              student: student,
                              className: classNames[student.classId] ??
                                  student.className ??
